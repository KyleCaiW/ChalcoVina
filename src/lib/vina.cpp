...
void Vina::set_ligand_from_string(const std::string& ligand_string) {
	...
	/*****************************************************************************************/
	// Re-set chalcogen weights after model reset to ensure they're not lost
	if (m_weight_interchb_O != 0.0 || m_weight_interchb_S != 0.0 || m_weight_interchb_N != 0.0) {
		m_model.set_interchb_weights(m_weight_interchb_O, m_weight_interchb_S, m_weight_interchb_N);
	}
	if (m_weight_intrachb_O != 0.0 || m_weight_intrachb_S != 0.0 || m_weight_intrachb_N != 0.0) {
		m_model.set_intrachb_weights(m_weight_intrachb_O, m_weight_intrachb_S, m_weight_intrachb_N);
	}
	/*****************************************************************************************/
...
/*****************************************************************************************/
void Vina::set_interchb_weights(double w_O, double w_S, double w_N) {
    m_weight_interchb_O = w_O;
    m_weight_interchb_S = w_S;
    m_weight_interchb_N = w_N;
	m_model.set_interchb_weights(w_O, w_S, w_N);
}

void Vina::set_intrachb_weights(double w_O, double w_S, double w_N) {
	m_weight_intrachb_O = w_O;
	m_weight_intrachb_S = w_S;
	m_weight_intrachb_N = w_N;
	m_model.set_intrachb_weights(w_O, w_S, w_N);
}
/*****************************************************************************************/
...
std::vector<double> Vina::score(double intramolecular_energy) {
	...
	if (m_sf_choice == SF_VINA || m_sf_choice == SF_VINARDO) {
		...
		/*****************************************************************************************/
		// Initialize gradients vector with zeros for all atoms
		gradients.resize(m_model.num_atoms());
		std::fill(gradients.begin(), gradients.end(), vec(0, 0, 0));

		// Calculate CHB energy only if any weight is not zero
		if (m_weight_interchb_O != 0.0 || m_weight_interchb_S != 0.0 || m_weight_interchb_N != 0.0) {
...
void Vina::global_search(const int exhaustiveness, const int n_poses, const double min_rmsd, const int max_evals) {
	...
	/*****************************************************************************************/
	auto compute_intramolecular_energy = [&](const output_type& pose) {
		m_model.set(pose.c);
		if (m_no_refine || !m_receptor_initialized)
			return m_model.eval_intramolecular(m_precalculated_byatom, m_grid, authentic_v);
		return m_model.eval_intramolecular(m_precalculated_byatom, m_non_cache, authentic_v);
	};
	/*****************************************************************************************/
	...
	if (!poses.empty()) {
		...
		if (m_sf_choice == SF_VINA || m_sf_choice == SF_VINARDO) {
			...
			/*****************************************************************************************/
			intramolecular_energy = compute_intramolecular_energy(poses[0]);
			/*****************************************************************************************/
		}
		...
		do {
			...
			if (m_sf_choice == SF_VINA || m_sf_choice == SF_VINARDO) {
				double updated_intramol = compute_intramolecular_energy(poses[0]);
				if (std::fabs(updated_intramol - intramolecular_energy) > 1e-7) {
					intramolecular_energy = updated_intramol;
					rescored = true;
				}
			}
		} while (rescored);
