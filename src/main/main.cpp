...
#################################################################\n";
	...
	try {
		...
		/*****************************************************************************************/
		double weight_interchb_O  =  0.4;
		double weight_interchb_S  =  0.7;
		double weight_interchb_N  =  0.0;
		double weight_intrachb_O  =  1.5;
		double weight_intrachb_S  =  0.0;
		double weight_intrachb_N  =  1.5;
		/*****************************************************************************************/
		...
		search_area.add_options()
			...
			/*****************************************************************************************/
			("chbmap", value<std::string>(&chbmap), "chalcogen bond map file (.map format)")
			/*****************************************************************************************/
		...
		advanced.add_options()
			...
			/*****************************************************************************************/
			("weight_interchb_O", value<double>(&weight_interchb_O)->default_value(weight_interchb_O),    "intermolecular S...O chalcogen bond weight")
			("weight_interchb_S", value<double>(&weight_interchb_S)->default_value(weight_interchb_S),    "intermolecular S...S chalcogen bond weight")
			("weight_interchb_N", value<double>(&weight_interchb_N)->default_value(weight_interchb_N),    "intermolecular S...N chalcogen bond weight")
			("weight_intrachb_O", value<double>(&weight_intrachb_O)->default_value(weight_intrachb_O),    "intramolecular S...O chalcogen bond weight")
			("weight_intrachb_S", value<double>(&weight_intrachb_S)->default_value(weight_intrachb_S),    "intramolecular S...S chalcogen bond weight")
			("weight_intrachb_N", value<double>(&weight_intrachb_N)->default_value(weight_intrachb_N),    "intramolecular S...N chalcogen bond weight")
			/*****************************************************************************************/
		...
		if (sf_name.compare("vina") == 0) {
			...
			/*****************************************************************************************/
			// Initialize CHB map if chbmap parameter is provided
			if (vm.count("chbmap")) {
				if (verbosity > 0) {
					std::cout << "Initializing chalcogen bond map from: " << chbmap << "\n";
				}
				if (!CHBTableManager::load_maps_from_file(chbmap)) {
					weight_interchb_O = 0.0;
					weight_interchb_S = 0.0;
					weight_interchb_N = 0.0;
					std::cerr << "WARNING: Failed to initialize chalcogen bond map from: " << chbmap << "\n";
					std::cerr << "WARNING: Chalcogen bond scoring will be disabled.\n";
				} else if (verbosity > 0) {
					std::cout << "Successfully initialized chalcogen bond map.\n";
				}
			} else {
				weight_interchb_O = 0.0;
				weight_interchb_S = 0.0;
				weight_interchb_N = 0.0;
				if (verbosity > 0) {
					std::cout << "No chalcogen bond map specified. Chalcogen bond scoring will be disabled.\n";
				}
			}
			// Set interchb weights after validation
			v.set_interchb_weights(weight_interchb_O, weight_interchb_S, weight_interchb_N);
			v.set_intrachb_weights(weight_intrachb_O, weight_intrachb_S, weight_intrachb_N);
			/*****************************************************************************************/
